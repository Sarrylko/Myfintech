...
export default function TransactionsPage() {
  ...
  return (
    ...
    <div>
      ...
      {receiptTxn && (
        <ReceiptModal
          transactionId={receiptTxn.id}
          transactionAmount={Math.abs(parseFloat(String(receiptTxn.amount)))}
          transactionName={receiptTxn.name}
          onClose={() => setReceiptTxn(null)}
          onConfirmed={() => {
            setTransactions((prev) =>
              prev.map((t) => t.id === receiptTxn.id ? { ...t, has_splits: true } : t)
            );
            setReceiptTxn(null);
          }}
        />
                  ...
                  <td className="px-5 py-3.5 text-right">
                    <div className="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition">
                      <button
                        ...
                        className={`w-6 h-6 rounded-md flex items-center justify-center transition ${txn.receipt ? "text-green-500 hover:bg-green-50" : "text-gray-300 hover:text-amber-500 hover:bg-amber-50"}`}
                        title={txn.receipt ? "View receipt" : "Attach receipt"}
                      >
